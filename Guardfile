guard :coffeescript, :input => 'app/assets/coffee', :output => 'public/assets'

guard :concat, :type => "css", :files => %w[application custom/app custom/mixins custom/site custom/skin custom/variables site], :input_dir => "public/assets", :output => "public/assets/styles.min"

guard :concat, :type => "js", :files => %w[], :input_dir => "public/assets", :output => "public/assets/scripts.min"

guard :less, :all_on_start => true, :all_on_start => false, :output => 'public/assets' do
...
guard :refresher do
  ...
  watch('public/assets/styles.min.css') do |m|
    css = File.read(m[0])
    File.open(m[0], 'w') { |file| file.write(CSSMin.minify(css)) }
  end
  watch('public/assets/scripts.min.js') do |m|
    js = File.read(m[0])
